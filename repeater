...
function HandleMIDI(e) {
    ...
    if (e instanceof NoteOn) {
        ...
        var wait = 0;
 	     ...
 	     //     e.sendAfterMilliseconds(6000);
 	 	    			 wait += Math.pow(GetParameter("Time Multiplier"),i + 1) * 60000 * getTime(GetParameter("Time")) / info.tempo;     
 	         		
 	     	  		 e.sendAfterMilliseconds(wait);
...
var PluginParameters = [{name:"Time", type:"menu", 
                        ...
                        unit:"%", type:"linear" },
                        {name:"Time Multiplier", defaultValue:1, minValue:0.5, maxValue:2, 
                         numberOfSteps:108, type:"linear"},
                         ];
